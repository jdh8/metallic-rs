...
fn test_atan2pi() {
    ...
    common::test_bivariate_cases(metallic::atan2pi, core_math::atan2pi, grid.chain(wide));
}

/// Correct-rounding gate on CORE-MATH's hard-to-round corpus.
#[test]
fn test_atan2pi_worst_cases() {
    common::test_worst_bivariate("atan2pi", metallic::atan2pi, core_math::atan2pi);
}

/// Faithful-rounding floor (≤ 1 ulp) on that same corpus.
#[test]
fn test_atan2pi_worst_faithful() {
    common::test_worst_faithful_bivariate("atan2pi", metallic::atan2pi, core_math::atan2pi, 1);
}
...
#[cfg(feature = "mpfr")]
#[test]
fn test_atan2pi_vs_mpfr() {
    let cr = |y: f64, x: f64| {
        rug::Float::with_val(200, y)
            .atan2_pi(&rug::Float::with_val(200, x))
            .to_f64()
    };
