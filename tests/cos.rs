...
fn test_cos() {
    ...
    common::test_univariate_cases(metallic::cos, core_math::cos, dense.chain(wide).chain(bits));
}

/// Correct-rounding gate on CORE-MATH's hard-to-round corpus (RED until cos is
/// correctly rounded — issue #6).
#[test]
fn test_cos_worst_cases() {
    common::test_worst_univariate("cos", metallic::cos, core_math::cos);
}

/// Faithful-rounding floor (≤ 1 ulp) on that same corpus.
#[test]
fn test_cos_worst_faithful() {
    common::test_worst_faithful("cos", metallic::cos, core_math::cos, 1);
}
