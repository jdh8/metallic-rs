...
/// Correct-rounding gate on CORE-MATH's hard-to-round corpus (RED until acosh is
/// correctly rounded — issue #6).
#[test]
fn test_acosh_worst_cases() {
    common::test_worst_univariate("acosh", metallic::acosh, core_math::acosh);
}

/// Faithful-rounding floor (≤ 1 ulp) on that same corpus.
#[test]
fn test_acosh_worst_faithful() {
    common::test_worst_faithful("acosh", metallic::acosh, core_math::acosh, 1);
}
