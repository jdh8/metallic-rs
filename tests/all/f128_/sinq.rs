...
// CORE-MATH has not shipped `sinq` yet — its binary128 sine is a corpus with
...
/// Size of `tests/cases/sinq.wc` (kept in sync with the generator).
const CORPUS_LEN: usize = 95_321;
...
fn test_sinq_corpus() {
    ...
    common::truncate_errors(cases.into_iter().filter_map(|[x, want]| {
        let got = metallic::sinq(x);
        (!got.is(&want)).then(|| println!("sinq({x:?}) = {got:?} != {want:?} (correct)"))
    }));
}

/// Every `f64` is a binary128 value, and its correctly rounded `f64` sine can
/// differ from the binary128 one rounded down to `f64` by at most one ulp
/// (the double rounding).  With CORE-MATH's `sin` as the oracle this checks
...
fn test_sinq_vs_f64() {
    common::truncate_errors((0..SAMPLE_COUNT).filter_map(|i| {
        ...
        let got = metallic::sinq(x as f128) as f64;
        let want = core_math::sin(x);
        (common::ulp_error_f64(got, want) > 1)
            .then(|| println!("sinq({x:e}) = {got:e} != {want:e}"))
    }));
}
...
fn test_sinq_odd() {
    common::truncate_errors((0..SAMPLE_COUNT).filter_map(|i| {
        let x = wide(i);
        let (got, want) = (metallic::sinq(-x), -metallic::sinq(x));
        (!got.is(&want)).then(|| println!("sinq({:?}) = {got:?} != {want:?}", -x))
    }));
}

#[test]
fn test_sinq_special() {
    assert!(metallic::sinq(0.0).is(&0.0));
    assert!(metallic::sinq(-0.0).is(&-0.0));
    assert!(metallic::sinq(core::f128::consts::FRAC_PI_2).is(&1.0));
    assert!(metallic::sinq(-core::f128::consts::FRAC_PI_2).is(&-1.0));
    assert!(metallic::sinq(f128::INFINITY).is_nan());
    assert!(metallic::sinq(f128::NEG_INFINITY).is_nan());
    assert!(metallic::sinq(f128::NAN).is_nan());
    // sin(x) rounds back to x below the cubic term's reach.
    let least = f128::from_bits(1);
    assert!(metallic::sinq(least).is(&least));
    assert!(metallic::sinq(-least).is(&-least));
    assert!(metallic::sinq(f128::MIN_POSITIVE).is(&f128::MIN_POSITIVE));
}
...
fn oracle(x: f128) -> f128 {
    ...
    metallic::f128_mpfr::cr_unop(x, |y| y.sin_round(Nearest))
}
...
#[cfg(feature = "mpfr")]
#[test]
fn test_sinq_vs_mpfr() {
    for sampler in [direct, reduced, wide, near_multiple] {
        common128::mpfr_sweep_univariate_f128(metallic::sinq, oracle, sampler, SAMPLE_COUNT);
    }
}
