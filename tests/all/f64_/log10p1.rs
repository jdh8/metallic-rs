...
fn test_log10p1() {
    ...
    common::test_univariate_cases(metallic::log10p1, core_math::log10p1, bits.chain(near_zero));
}
...
fn test_log10p1_wide_band() {
    ...
    common::test_univariate_cases(metallic::log10p1, core_math::log10p1, cases);
}

/// Correct-rounding gate on CORE-MATH's hard-to-round corpus.
#[test]
fn test_log10p1_worst_cases() {
    common::test_worst_univariate("log10p1", metallic::log10p1, core_math::log10p1);
}

/// Faithful-rounding floor (≤ 1 ulp) on that same corpus.
#[test]
fn test_log10p1_worst_faithful() {
    common::test_worst_faithful("log10p1", metallic::log10p1, core_math::log10p1, 1);
}
...
#[cfg(feature = "mpfr")]
#[test]
fn test_log10p1_vs_mpfr() {
    let cr = |x: f64| rug::Float::with_val(200, x).log10_1p().to_f64();
    let sampler = |i: u64| {
    ...
    common::mpfr_sweep_univariate(metallic::log10p1, cr, sampler, 2_000_000);
}

/// The exact cases `log10p1(10ⁿ − 1) = n` must come out exactly integer.
#[test]
fn test_log10p1_exact_cases() {
    for n in 1_i32..=15 {
        let x = f64::powi(10.0, n) - 1.0; // exact: 10ⁿ − 1 < 2⁵³
        assert_eq!(metallic::log10p1(x), f64::from(n), "log10p1(10^{n} - 1)");
    }
}
