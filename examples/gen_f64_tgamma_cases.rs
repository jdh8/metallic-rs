//! Generate `tests/cases/f64_tgamma.wc`: the hard-to-round regression corpus for
//! [`metallic::f64::tgamma`].
//!
//! `tgamma` has no correctly-rounded `f64` reference in the `core-math` crate
//! (only `tgammaf`), so MPFR is the oracle.  Each sampled `z` is evaluated at
//! [`PREC`] bits; we keep the tiny fraction whose true `Γ(z)` lands within
...
fn main() {
    ...
    out.push_str("# Hard-to-round cases for metallic::f64::tgamma(z).\n");
    out.push_str("# Columns: z, correctly-rounded tgamma(z).\n");
    out.push_str(
        "# Generated by `cargo run --release --features mpfr --example gen_f64_tgamma_cases`.\n",
    );

    let mut mismatches = 0;
    for &z in &cases {
        let want = oracle(z).to_f64();
        let got = metallic::f64::tgamma(z);
        if got.to_bits() != want.to_bits() {
            mismatches += 1;
            eprintln!("MISMATCH tgamma({z:e}) = {got:e} != {want:e} (correct)");
        }
        writeln!(out, "{}, {}", hexf64(z), hexf64(want)).unwrap();
    }

    let path = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/cases/f64_tgamma.wc");
    ...
    assert_eq!(
        mismatches, 0,
        "metallic::f64::tgamma is not correctly rounded"
    );
}
