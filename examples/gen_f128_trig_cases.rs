...
fn convergents() -> (Vec<f128>, Vec<f128>) {
    ...
    for e in 0..=16383i32 {
        ...
        let mut fast_best = [(i32::MAX, 0u128); 3];
        ...
        loop {
            ...
            if e >= 114 && e % 7 == 0 && q2 != 0 {
                let error =
                    (Integer::from(&q2 * &numerator) - Integer::from(&p2 * &denominator)).abs();
                let t = denominator.significant_bits() as i32 - error.significant_bits() as i32;
                if (8..=57).contains(&t) {
                    for (target, best) in [16i32, 36, 56].into_iter().zip(&mut fast_best) {
        ...
        for (_, q) in fast_best {
            if q != 0 {
                let x = Float::with_val(113, q) << (e - 112);
                if x.get_exp().unwrap() > 114 {
                    fast_window.push(x.to_f128());
                }
            }
        }
...
fn write(path: &str, name: &str, sections: &[(&str, &[f128])], f: fn(f128) -> f128) {
    ...
    for (title, inputs) in sections {
        ...
        for &x in *inputs {
            ...
            let want = f(x);
            let got = match name {
                "sinq" => metallic::sinq(x),
                "cosq" => metallic::cosq(x),
                _ => metallic::tanq(x),
            };
            mismatches +=
    ...
    eprintln!(
        "wrote {path}: {} cases, {mismatches} implementation mismatches",
        seen.len()
    );
}

fn upstream(name: &str) -> Vec<f128> {
    let root = std::env::var("CORE_MATH_SYS").unwrap_or_else(|_| "../core-math-sys".into());
    let path = format!("{root}/vendor/src/binary128/{name}/{name}q.wc");
    let text = std::fs::read_to_string(&path).expect("CORE-MATH corpus");
    let inputs: Vec<_> = text
        .lines()
        .filter_map(|line| {
            let line = line.split('#').next().unwrap().trim();
            (!line.is_empty()).then(|| common128::parse_f128(line).expect("CORE-MATH input"))
        })
        .collect();
    assert!(!inputs.is_empty(), "empty {path}");
    inputs
}

fn main() {
    let edges = edges();
    let (family, fast_window) = convergents();
    let sin_upstream = upstream("sin");
    let cos_upstream = upstream("cos");
    ...
    write(
        ...
        &[
            ...
            (
                "near multiples of pi/2 above 2^114, inside the fast window",
                &fast_window,
            ),
