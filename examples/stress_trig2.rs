fn main() {
    ...
    let mut bad = [0u64;3]; let mut worst=[0i64;3];
    let mut n=0u64;
    let mut i: u64 = 0;
        ...
        if x.is_finite() {
            ...
            for (k,(f,g)) in [
                ...
                if !(a.is_nan()&&b.is_nan()) {
                    ...
                    if d!=0 { bad[k]+=1; if d>worst[k]{worst[k]=d; if bad[k]<=3 {eprintln!("  k{k} {x:e}: {a:e} vs {b:e} ({d})");}} }
                }
            }
        }
        i += (1u64<<33)-1337;
    }
    eprintln!("bit-sweep n={n}: sin bad={} (worst {}), cos bad={} (worst {}), tan bad={} (worst {})",
