...
**Historical measurements, not timings of the current checkout.** The measured source, dependency locks and machine conditions are recorded below. See the [measurement policy](benchmarks/README.md) for later campaigns and refresh guidance.

Later [f64 optimization measurements](benchmarks/2026-09-05/f64-optimization/README.md) and [f32 optimization measurements](benchmarks/2026-09-05/f32-optimization/README.md) record subsequent improvements with paired before/after results.
...
[The report generator](tools/benchmark_report.py) validates 92/92/20 public-function coverage, all nine supplementary bands in each default run, archived lock hashes, matching archived source manifests, and consumption of every recorded lane. Raw Criterion files and build/run logs remain in the target directories recorded in each snapshot.

## Static analysis

[ANALYSIS.md](ANALYSIS.md) complements these measurements with per-path cycle
estimates, accurate-leg coverage and table footprints. Its representative inputs
and scheduling models do not predict every workload's ranking; performance
claims use measured same-run ratios. See the [measurement policy](benchmarks/README.md)
for refresh cadence and hardware coverage.
