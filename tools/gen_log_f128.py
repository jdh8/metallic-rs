#!/usr/bin/env python3
"""Generate `src/f128_/log_tables.rs`: the fixed-point constants of the
binary128 logarithm.
...
def log_table(name, step, count, doc):
    ...
    for j in range(count):
        ...
        value = int(nint(-log(r) * mpf(2) ** FRAME))
...
for level, (step, count) in enumerate(LEVELS):
    log_table(
        ...
        step,
        count,
        f"-log(RECIP{level}[j]/2^{RECIP}), scaled by 2^{FRAME}.",
    )

crude_table()

print("""
/// `1/(k+1)` for `k = 0..=13`, scaled by 2^255: the Taylor coefficients of
/// `log(1+z)/z = sum_k (-1)^k COEF[k] z^k`.  The fast leg stops at `k = 6`,
/// where the tail is below 2^-152 for `|z| < 2^-18.7`.
#[rustfmt::skip]
pub const COEF: [[u128; 2]; 14] = [""")
for k in range(1, 15):
    print(f"    {limbs(int(nint(mpf(2) ** 255 / k)), 2)},")
print("];")
