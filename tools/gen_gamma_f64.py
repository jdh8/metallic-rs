...
The accurate path runs entirely in triple-double (≈2⁻¹³⁵): a degree-38 central
minimax (`TGAMMA_TD`) walked through a triple-double recurrence, which clears the
corpus's ≈2⁻⁶⁰ near-ties that the old double-double path (≈2⁻⁹⁹ after the long
recurrence) mis-rounded.  Correctness is verified against the MPFR worst-case
corpus and CORE-MATH 1.1.1.
"""
...
def fit(f, a, b, target, nmax=32):
    for n in range(4, nmax):
        coeffs, err = chebyfit(f, [a, b], n, error=True)
        if err < target:
            return list(reversed(coeffs)), err, n - 1
    coeffs, err = chebyfit(f, [a, b], nmax, error=True)
    return list(reversed(coeffs)), err, nmax - 1
...
def td3(x):
    """Split an mpf into a non-overlapping triple-double (hi, mid, lo)."""
    hi = f64(x)
    r = x - mpf(hi)
    mid = f64(r)
    r = r - mpf(mid)
...
def print_td_array(name, coeffs, doc):
    print(f"\n/// {doc}")
    print(f"const {name}: [TripleDouble; {len(coeffs)}] = [")
    for c in coeffs:
        hi, mid, lo = td3(c)
        print(f"    TripleDouble {{ high: {hexf(hi)}, mid: {hexf(mid)}, low: {hexf(lo)} }},")
    print("];")
...
# --- Γ(2.875 + d) minimax in TRIPLE-double for the accurate path ------------
# The accurate path is triple-double: a double-double recurrence degrades to
# ≈2⁻⁹⁹ over the long ∏ (up to ~168 factors), straddling the corpus's hardest
# ≈2⁻⁶⁰ near-ties.  Carrying the central minimax AND the recurrence products in
# triple-double reaches ≈2⁻¹³⁵, clearing every tie with a > 70-bit margin.  Fit
# to a tighter target so the polynomial-approximation error is not the
# bottleneck.  (`TARGET` is retained for the fast-leg table fits below.)
TD_TARGET = mpf(2) ** -136
td_coeffs, td_err, td_deg = fit(lambda d: gamma(CENTER + d), mpf("-0.5"), mpf("0.5"), TD_TARGET, nmax=44)
print(f"\n// Γ(2.875 + d), d ∈ [−½, ½] (triple-double): degree {td_deg}, err 2^{float(mp.log(td_err,2)):.1f}")
print_td_array("TGAMMA_TD", td_coeffs,
               f"Accurate `Γ(2.875 + d)` minimax, triple-double (degree {td_deg}, ≈2⁻¹³⁶)")
