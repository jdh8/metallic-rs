...
#[inline]
fn asin_reflect_node(a: f64) -> Sum {
    sqrt_dd(Sum {
        high: (1.0 - a) * 0.5,
        low: 0.0,
    })
...
#[inline]
fn asin_fast(a: f64) -> Sum {
    if a < 0.5 {
        asin_kernel_fast(Sum { high: a, low: 0.0 })
    } else if a == 1.0 {
...
#[inline]
fn acos_fast(x: f64) -> Sum {
    let a = x.abs();
    if a < 0.5 {
        FRAC_PI_2 + neg(asin_kernel_fast(Sum { high: x, low: 0.0 }))
    } else if a == 1.0 {
