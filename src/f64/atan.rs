...
/// Leading 6 terms of `asin(t)/t = Σ aₖ·u^k` (`u = t²`), as double-doubles,
...
const ASIN_LEADS: [DoubleDouble; 6] = [
    DoubleDouble {
        high: 1.0,
        low: 0.0,
    },
    ...
    DoubleDouble {
        high: 0.022372159090909092,
        low: -9.462128050782583e-19,
    },
];
...
const ASIN_TAIL: [f64; 14] = [
    ...
    0.013964843750000804,
    0.011551800895931238,
    0.009761609550423002,
    0.008390334684834052,
    0.007312561449572762,
    0.006446484399297868,
    0.0057500372031336925,
    0.0050580153522191305,
    0.005293266753521957,
    0.0013359273995765784,
    0.012790596886414774,
    -0.013373166459419364,
    0.01980676633658218,
];
...
/// `asin(t) = t·B(t²)`.  Mirrors the f32 `asin_near_zero` kernel: the 6 leading
...
fn asin_kernel_fast(t: DoubleDouble) -> DoubleDouble {
    ...
    let u2 = u * u;
    let u6 = u2 * u2 * u2;
    // 2-way Estrin of `1 + a1·w + … + a5·w⁵` (ASIN_LEADS): same op count as Horner
    // but a shorter critical path (the three pairs evaluate in parallel).  The `u⁶`
    // remainder (ASIN_TAIL) is plain `f64`.
    let w2 = w * w;
    let p0 = ASIN_LEADS[0] + w * ASIN_LEADS[1];
    let p1 = ASIN_LEADS[2] + w * ASIN_LEADS[3];
    let p2 = ASIN_LEADS[4] + w * ASIN_LEADS[5];
    let bracket = (p0 + w2 * (p1 + w2 * p2))
        + DoubleDouble {
            high: u6 * crate::poly(u, &ASIN_TAIL),
            low: 0.0,
        };
    bracket * t
}
