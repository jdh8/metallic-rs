use super::uint::{add_384, cmp_384, extract_u128, leading_zeros_384, mhi, shl_384, sqr_384};
use super::{BIAS, EXP_MASK, EXP_SHIFT, IMPLICIT_BIT, QUIET_BIT, SIGN_MASK, split};
use core::cmp::Ordering;

/// The Euclidean norm, √(x² + y²).
///
/// Unlike the transcendental functions there is no Ziv fallback here: with the
/// exponent gap capped at 56, the sum of squares `ma² · 2^(2·dn) + mb²` is an
/// *exact* 384-bit integer `V`, and the last bit follows from two exact
/// comparisons against it — one pinning the significand to `⌊√V⌋`, one
...
pub fn hypotq(x: f128, y: f128) -> f128 {
    ...
    // The same cutoff caps the shift below at 112 bits, keeping V inside the
    // 384-bit window.
    if dn > 56 {
        return f128::from_bits(a);
    }
    let v = add_384(shl_384(sqr_384(ma), 2 * dn as u32), sqr_384(mb));
