    ...
    "Benchmark": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "3bbc9bde2533869773fcbc683cf5ea7853db8c42",
          "message": "Speed up f64 lgamma reflection by folding the recurrence log into the sine\n\nThe `z < ½` reflection computes `ln π − ln|sin πz| − ln Γ(1−z)`.  When `1−z`\nlands in the central-table band, `ln Γ(1−z)` is a table value `cell_eval(d)`\nplus the recurrence's `ln ∏` — so the fast leg took *two* logarithms there\n(`ln|sin πz|` and `ln ∏`) where CORE-MATH's reflection takes one.\n\nFold the recurrence product into the sine before the logarithm: a single\n`ln(|sin πz|·∏^±1)` now replaces both logs.  Combining is at least as accurate\nas the two separate logs (one transcendental rounding instead of two, and\n`|ln(ab)| ≤ |ln a|+|ln b|`), so the existing `LGAMMA_TABLE_ERR` gate still\ncertifies the leg — `tests/lgamma.rs`'s worst-case corpus passes bit-for-bit.\n\nlgamma_reflect (`[-10,-2]` band): 65.1 → 55.6 ns, 2.16× → 1.84× vs CORE-MATH.\nThe Stirling sub-band (`1−z ≥ 8`) keeps the two-log form (its `ln w` is scaled\nby `w−½`, not addable to the sine) — at structural parity with CORE-MATH there.\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>",
          "timestamp": "2026-06-17T22:15:43+08:00",
          "tree_id": "1fd473142912cc6515e4ea84030fc4e9d6c5abb0",
          "url": "https://github.com/jdh8/metallic-rs/commit/3bbc9bde2533869773fcbc683cf5ea7853db8c42"
        },
        "date": 1781707674309,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "32d33f25ba59debabe45fa2c70a3f1cd00950f34",
          "message": "Speed up f64 log1p mid-|x| with a relative-gated polynomial leg\n\nThe reduction path's absolute Ziv gate (reused from `ln`, 2⁻⁶³) is coarse\nrelative to a small result `ln(1+x)`, so for `2⁻⁸ ≤ |x| < 2⁻⁴` it forced the\nexpensive 128-bit `dint` fallback on a large fraction of inputs — that sub-band\nran 2.36× CORE-MATH (38.5 ns vs 16.3 ns).\n\nCover `[2⁻⁸, 2⁻⁴)` with a polynomial leg instead, mirroring CORE-MATH's\n`|x| < 2⁻⁴` branch: carry the leading `x − x²/2` exactly (one `fma`, its\nresidual joining the low word) so only the `x³·f` correction carries error and\nthe gate scales with `x³` (`LN1P_WIDE_ZIV_SCALE = 2⁻⁵⁰`, ~4× margin) — the\nfallback all but vanishes.  `f` is the degree-13 Taylor tail\n`(ln(1+x) − x + x²/2)/x³` (`LN1P_F_WIDE`).  The existing degree-7 small path\n(`|x| < 2⁻⁸`) and the reduction path (`|x| ≥ 2⁻⁴`) are untouched, so `ln` /\n`log2` / `log10` are unaffected.\n\nThat sub-band drops 2.36× → 1.15×, and full-range log1p\n(`Exponents(-50..=-1)`) 1.11× → 1.06× CORE-MATH.  `test_log1p_worst_cases`\n(the strict correct-rounding corpus) and a dense 2.1M-sample `[2⁻⁸, 2⁻⁴)` sweep\nagainst the `core-math` oracle round bit-exactly; `log`/`log2`/`log10` tests\nunchanged.\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>",
          "timestamp": "2026-06-19T01:42:03+08:00",
          "tree_id": "ba921f65dbf7f9d58c35c45ff6ddff9f0a818cae",
          "url": "https://github.com/jdh8/metallic-rs/commit/32d33f25ba59debabe45fa2c70a3f1cd00950f34"
        },
        "date": 1781808102912,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "10f8830feafef0034a88759a6b30663527851f03",
          "message": "Give f64 log2 a native base-2 fast leg\n\nlog2 computed ln_fast_raw(x) × log2(e) — a closing double-double multiply\n(plus ln's e·LN2_LO fold) on the gate path, paying a conversion tax where\nCORE-MATH's dedicated log2 exploits base 2: the exponent join is exact.\n\nNew LOG2_CELLS table (same exact-z reciprocal lattice as LN_CELLS, l words\nin base 2 on the 2^-42 grid so e + l1 is exact as a plain add), generated\nby tools/gen_log2_exact_f64.py, which re-verifies the per-cell Fast2Sum\norderings against max|zh| = max|z|·LOG2_E_HI·(1+2^-53) — the leg's linear\nterm z·log2(e) is no longer exact, so it carries its FMA residual and the\nz·LOG2_E_LO sliver in the low word, and the base-e tail polynomial is\nreused scaled by LOG2_E_HI on the way in.  Error budget stays the base-e\nleg's <2^-66 scaled by log2(e) plus ~2^-67 of new low-word rounding;\nLOG2_ZIV_EPS and the dint::log2_accurate fallback are unchanged.\n\nln_exact_reduce is parameterized over the cell table (exact_reduce_in);\nthe base-e callers are untouched (log same-run ratio 0.994, unchanged).\n\nmetallic::log2 15.56 → 15.19 ns (−2.2%, p = 0.00 on a calm box), same-run\nCORE-MATH ratio 1.07 → 1.05.  Verified: log2 dense + bit sweeps and the\nworst-case corpus bit-exact, plus a new test_log2_vs_mpfr independent\n2M-point sweep (log2.rs previously had no MPFR gate).\n\nCo-Authored-By: Claude Fable 5 <noreply@anthropic.com>\nClaude-Session: https://claude.ai/code/session_0146rStmW9uuSqfNurfTQWYd",
          "timestamp": "2026-07-02T13:14:36+08:00",
          "tree_id": "688e2a5a14a8a9397a310c78ec2902abada3aa37",
          "url": "https://github.com/jdh8/metallic-rs/commit/10f8830feafef0034a88759a6b30663527851f03"
        },
        "date": 1782976236025,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "99f740b5a85961620ebd282f2f91a6f9ebc56b71",
          "message": "Certify the log-family Ziv gates with MPFR soundness proofs\n\nThe recent fast legs (log2's native base-2 leg, log1p's [2⁻⁸, 2⁻⁴)\npolynomial leg) shipped with prose-only error budgets — log.rs was the\nonly kernel file with no ziv_soundness module.  Extract the gated legs\n(log2_fast_raw, log1p_small_eval, log1p_wide_eval, log1p_table_raw) and\nprove all six gates sound vs 250-bit MPFR: worst |err|/gate = ln 0.080,\nlog2 0.068, log10 0.036, log1p small 0.069, wide 0.222, table 0.093.\nAdd the previously-uncommitted 2.1M-point wide-band sweep vs core-math\nto default tests, and test_log1p_vs_mpfr / test_log10_vs_mpfr — every\nlog-family fn now has the independent MPFR gate.\n\nCo-Authored-By: Claude Fable 5 <noreply@anthropic.com>",
          "timestamp": "2026-07-03T06:56:42+08:00",
          "tree_id": "e274d862084745ec22250299da8385788f2c8adc",
          "url": "https://github.com/jdh8/metallic-rs/commit/99f740b5a85961620ebd282f2f91a6f9ebc56b71"
        },
        "date": 1783033889404,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "e0b3ced3f3c814f195744e8df700e69a38d14c36",
          "message": "Silence the deliberate suboptimal_flops sites so clippy is green on main\n\nFive raw multiply-add shapes are intentional, not missed FMAs: cbrt's\nHalley/Newton products and lgamma's digamma correction overlap in the\nOoO window and their exact roundings are what the certified error\nbudgets assume, and the two test-module sites mirror analytic bounds\nor generate test inputs. Targeted allows with the reasons keep the\ncrate-level deny meaningful and stop future sessions from\nmisattributing a red  to their own changes.\n\nCo-Authored-By: Claude Fable 5 <noreply@anthropic.com>",
          "timestamp": "2026-07-03T20:12:26+08:00",
          "tree_id": "b3c7ebfae32d63687445bc1d0a7594a1efc723c0",
          "url": "https://github.com/jdh8/metallic-rs/commit/e0b3ced3f3c814f195744e8df700e69a38d14c36"
        },
        "date": 1783082408264,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "b44307258074517ec4b7549bac2348dce7c0cbe7",
          "message": "Compare logf's hard ties as floats, not bits\n\nThe integer-dispatch commit turned logf's five tie checks into a `match` on\nu32 bit patterns. That regressed logf to 15.9 ns (ratio 1.41): comparing the\nbit pattern contends with log_lookup's own integer table/exponent work,\nwhereas comparing the float value `x` runs on the idle FP unit in parallel\nwith it (a branchless OR of the integer compares was no better). Compare `x`\ndirectly instead — log2f/log10f are unaffected (no five-way tie match).\n\nSame-run median vs CORE-MATH: 15.9 -> 12.83 ns, ratio 1.41 -> 1.11 (the\n13.16 ns / 1.19 baseline improves via the integer specials fold, kept).\nExhaustive 2^32 sweep vs the core-math oracle: green.\n\nCo-Authored-By: Claude Fable 5 <noreply@anthropic.com>",
          "timestamp": "2026-07-11T01:23:59+08:00",
          "tree_id": "b8223ab52d4158b75b33eec8f9a7a0440123dbc4",
          "url": "https://github.com/jdh8/metallic-rs/commit/b44307258074517ec4b7549bac2348dce7c0cbe7"
        },
        "date": 1783706948079,
        "tool": "cargo",
        "benches": [
          ...
          {
            "name": "metallic::acospi",
            "value": 29,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::acospi",
            "value": 28,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "metallic::acospif",
            "value": 11,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::acospif",
            "value": 11,
            "range": "± 0",
            "unit": "ns/iter"
          },
          ...
          {
            "name": "metallic::asinpi",
            "value": 25,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::asinpi",
            "value": 37,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "metallic::asinpif",
            "value": 11,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::asinpif",
            "value": 11,
            "range": "± 0",
            "unit": "ns/iter"
          },
          ...
          {
            "name": "metallic::atan2pi",
            "value": 74,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::atan2pi",
            "value": 79,
            "range": "± 1",
            "unit": "ns/iter"
          },
          {
            "name": "metallic::atan2pif",
            "value": 87,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::atan2pif",
            "value": 158,
            "range": "± 2",
            "unit": "ns/iter"
          },
          ...
          {
            "name": "metallic::atanpi",
            "value": 38,
            "range": "± 1",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::atanpi",
            "value": 38,
            "range": "± 1",
            "unit": "ns/iter"
          },
          {
            "name": "metallic::atanpif",
            "value": 14,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::atanpif",
            "value": 14,
            "range": "± 0",
            "unit": "ns/iter"
          },
          ...
          {
            "name": "metallic::compoundf",
            "value": 29,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::compoundf",
            "value": 35,
            "range": "± 0",
            "unit": "ns/iter"
          },
          ...
          {
            "name": "core_math::cospi",
            "value": 27,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "metallic::cospif",
            "value": 7,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::cospif",
            "value": 8,
            "range": "± 0",
            "unit": "ns/iter"
          },
          ...
          {
            "name": "metallic::exp10m1",
            "value": 16,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::exp10m1",
            "value": 23,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "metallic::exp10m1f",
            "value": 10,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::exp10m1f",
            "value": 10,
            "range": "± 0",
            "unit": "ns/iter"
          },
          ...
          {
            "name": "metallic::exp2m1",
            "value": 17,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::exp2m1",
            "value": 25,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "metallic::exp2m1f",
            "value": 11,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::exp2m1f",
            "value": 10,
            "range": "± 0",
            "unit": "ns/iter"
          },
          ...
          {
            "name": "metallic::log10p1",
            "value": 34,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::log10p1",
            "value": 32,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "metallic::log10p1f",
            "value": 20,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::log10p1f",
            "value": 26,
            "range": "± 0",
            "unit": "ns/iter"
          },
          ...
          {
            "name": "metallic::log2p1",
            "value": 32,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::log2p1",
            "value": 32,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "metallic::log2p1f",
            "value": 23,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::log2p1f",
            "value": 23,
            "range": "± 0",
            "unit": "ns/iter"
          },
          ...
          {
            "name": "core_math::rsqrt",
            "value": 20,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "metallic::rsqrtf",
            "value": 14,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::rsqrtf",
            "value": 15,
            "range": "± 0",
            "unit": "ns/iter"
          },
          ...
          {
            "name": "core_math::sinpi",
            "value": 25,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "metallic::sinpif",
            "value": 10,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::sinpif",
            "value": 9,
            "range": "± 0",
            "unit": "ns/iter"
          },
          ...
          {
            "name": "core_math::tanpi",
            "value": 24,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "metallic::tanpif",
            "value": 9,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::tanpif",
            "value": 9,
            "range": "± 0",
            "unit": "ns/iter"
          },
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "aed1c6ba63c4df789c495586596091103f4830ff",
          "message": "Make atanhf division-free with a lean two-log fast leg\n\natanhf ran (1+x)/(1-x) plus a second divide in the reduction for every\ninput.  It now dispatches on integer magnitude into three tiers:\n\n- |x| < 0x1.713744p-12: atanh(x) rounds to x itself (x^3/3 < 1/2 ulp;\n  the cut is exactly the first miss, m^3 = 3 in the 2^-12 binade,\n  verified by an MPFR scan).\n- |x| < 2^-5: the existing odd atanh kernel, division-free, well inside\n  its certified (sqrt(2)-1)/(sqrt(2)+1) range.  Cutting here instead of\n  at the full kernel range turns a 16%-taken random branch into a\n  3%-taken one -- the mispredicts were worth ~1.2 ns.\n- the rest: atanh(x) = (ln(1+x) - ln(1-x))/2 as a lean two-sided leg on\n  log_lookup's 65-cell geometry, all-integer front end (|x| in Q0.32\n  fixed point: 1+|x| parks straight into an f64 mantissa with no\n  exponent term; 1-|x| is its wrapping negation, normalized by one clz\n  whose count is the negated exponent).  hi words only, shared degree-3\n  Taylor tail; e*exp_hi + hi[j] is grid-exact.  Certified error < 2^-37\n  absolute (measured supremum vs the accurate tier 2^-38.3), gated by a\n  symmetric +-2^-36 rounding test (|err|/gate = 0.20, 4.9x margin);\n  fallback rate 0.07% (worst 0.8% at the band edge), landing in the old\n  ratio-reduction path kept verbatim as the accurate tier.\n\nExhaustive 2^32 sweep vs the core-math oracle: green (this is the\ncorrectness proof; no separate ziv_soundness module is needed at f32\nwhere every input is tested bit-exactly).\n\nSame-run bench (x86-64-v3, calm box): 9.67 -> 8.17 ns, CORE-MATH ratio\n1.29 -> 1.02 (CM 8.01 ns same run).  Structure follows CORE-MATH's\ncr_atanhf (two table-driven logs, no division); tables, tail, and gate\nare metallic's own.\n\nCo-Authored-By: Claude Fable 5 <noreply@anthropic.com>",
          "timestamp": "2026-07-11T18:51:33+08:00",
          "tree_id": "65de7de4ad096f1a22d221929d5111573b4910c1",
          "url": "https://github.com/jdh8/metallic-rs/commit/aed1c6ba63c4df789c495586596091103f4830ff"
        },
        "date": 1783769693253,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "e7fe56846c6c9f3a91c80587e41222062071bd5f",
          "message": "Correct the bench playbook: concurrent runs corrupt pair ratios\n\nAn 8-way concurrent f32 sweep on the 8-physical-core box inflated\nabsolute times up to 2× and skewed within-pair ratios in both\ndirections (atanhf 1.75 vs true 1.02, expm1f 0.64 vs true 1.06):\nhyperthread-sibling port contention hits a table-heavy arm and a\npoly-heavy arm differently.  Benches run strictly serially now.\n\nCo-Authored-By: Claude Fable 5 <noreply@anthropic.com>",
          "timestamp": "2026-07-11T20:18:28+08:00",
          "tree_id": "0e3ce386eb55c882b86f9f3088a09041eac2edb2",
          "url": "https://github.com/jdh8/metallic-rs/commit/e7fe56846c6c9f3a91c80587e41222062071bd5f"
        },
        "date": 1783774779650,
        "tool": "cargo",
        "benches": [
          ...
          {
            "name": "metallic::lgammaf_reflect",
            "value": 30,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::lgammaf_reflect",
            "value": 26,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "metallic::lgammaf_rational",
            "value": 15,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::lgammaf_rational",
            "value": 22,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "metallic::lgammaf_stirling",
            "value": 17,
            "range": "± 0",
            "unit": "ns/iter"
          },
          {
            "name": "core_math::lgammaf_stirling",
            "value": 17,
            "range": "± 0",
            "unit": "ns/iter"
          },
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "a23a0701feb47db062fde50ee4813611a050b643",
          "message": "Add correctly-rounded f64 compound = (1+x)^y\n\nFills the last C23 gap: compound existed only as f32 compoundf.  CORE-MATH\nships no binary64 compound, so — like tgamma/lgamma — it is MPFR-verified\nagainst a home-grown hard-to-round corpus.\n\nThe fast leg mirrors pow_fast with a 1+x front end (ln_dd_fast on the exact\n1+x double-double).  The accurate leg reuses pow's Dint/Qint cascade and\nexact/midpoint detector; since log_2/log_3 need an f64-precision base, it\nsplits ln(1+x) = ln(s.high) + ln1p(s.low/s.high) with a Newton-reciprocal\ncorrection.\n\nVerified: 30M bit-exact vs pow on the exact-1+x domain, 8M vs MPFR on the\nsmall-|x| region, a 2925-case frozen corpus, and the fast-leg Ziv soundness\n(worst |err|/slack = 0.3048, a 3.3x margin).\n\nCo-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>",
          "timestamp": "2026-07-11T22:32:09+08:00",
          "tree_id": "6550b5802ad4f412b9e70f62775b4da15cc656fc",
          "url": "https://github.com/jdh8/metallic-rs/commit/a23a0701feb47db062fde50ee4813611a050b643"
        },
        "date": 1783781400001,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "4c49029f13c4af9c3c96163a046147f100869f24",
          "message": "Add binary128 foundation and correctly rounded q roots",
          "timestamp": "2026-08-23T03:48:36+08:00",
          "tree_id": "249a72670a9134fb62022fa93b3076f9364241d8",
          "url": "https://github.com/jdh8/metallic-rs/commit/4c49029f13c4af9c3c96163a046147f100869f24"
        },
        "date": 1787429220617,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "a62b79d35984dfdd4d260318530b85026f7489c1",
          "message": "Optimize size of target/debug",
          "timestamp": "2026-08-23T06:16:36+08:00",
          "tree_id": "7fe08ca3e9f1f130e28e14ea9449f49c414f0649",
          "url": "https://github.com/jdh8/metallic-rs/commit/a62b79d35984dfdd4d260318530b85026f7489c1"
        },
        "date": 1787438024450,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "b5152540fe1f1406675cf7a2250691551fc77636",
          "message": "Add correctly rounded binary128 atan2q\n\nReduce on dyadic breakpoints: one float divide picks i = round(64·min/max)\nwithin 0.505, and since i/64 is a 6-bit dyadic both sides of\ntan(θ − atan(i/64)) are exact 128-bit integers (dn ≤ 7 whenever i ≥ 1).  A\nfloat-seeded Newton reciprocal — no integer division anywhere — yields the\nreduced tangent to 128 bits on the fast leg and 384 on the accurate one,\nalways from below so every residual stays nonnegative.  The atan(i/64) table\nand the quadrant offsets add back in the same top-limbs-shared 256/384 frame\nsplit as logq; the first-quadrant i = 0 band keeps atan(t) floating all the\nway down through subnormal results to underflow-to-zero, which only the\naccurate leg's rounder handles — the fast leg rounds on a fixed 15-bit guard\nand bails below f128::MIN_EXP.\n\nThe polynomial runs latency-first: the even and odd halves of the Taylor sum\nare independent Horner chains in T⁴ with 64-bit tails, merged after T³ has\nalready multiplied in.  Quadrant signs select by mask, not branch.\n\nConstants come from tools/gen_atan2_f128.py — atan(i/64), the offsets, the\nTaylor coefficients 1/(2k+3), and 3π/4 for the infinity edges — which also\naudits that every exact-ratio table sum clears its nearest rounding boundary\nby 2^-121.3 or more, so the 384-bit frame decides such inputs outright.\n\nGates: bit-exact vs core_math::atan2q on CORE-MATH's 269 282-pair corpus,\na 500k-pair MPFR precision-113 sweep, and the in-source ziv_soundness\ncertification: worst fast-leg |err|/gate = 0.0720 over 200 000 quadrant-mixed\nsamples, a 13.9× margin.  Same-run CORE-MATH ratio 1.24× (75.8 ns vs\n61.3 ns median, banded ±20 draws); the ablation trail and the lead for the\nremaining gap are recorded in the perf playbook.\n\nCo-Authored-By: Claude Fable 5 <noreply@anthropic.com>",
          "timestamp": "2026-08-24T04:54:06+08:00",
          "tree_id": "1da1fa2136901776528c48b96da8fbb41cd67e0f",
          "url": "https://github.com/jdh8/metallic-rs/commit/b5152540fe1f1406675cf7a2250691551fc77636"
        },
        "date": 1787519520014,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "6d551a73d3b2145ba2e23d05dc87c03d8826ce80",
          "message": "docs(skill): record the logq levers\n\nThe select-unswitch trap (LLVM cloning a Horner chain behind a 50/50\nsign branch), the normalized-discarded-field rounding pattern, and the\napproximate-high-product + wider-gate latency trade with its Estrin\nfollow-up, each with the measured deltas from the logq campaign\n(1.20x -> 1.00x).\n\nCo-Authored-By: Claude Fable 5 <noreply@anthropic.com>",
          "timestamp": "2026-08-24T21:51:35+08:00",
          "tree_id": "32651ff5d9a391eaf81ff1345743374163b16dbc",
          "url": "https://github.com/jdh8/metallic-rs/commit/6d551a73d3b2145ba2e23d05dc87c03d8826ce80"
        },
        "date": 1787590760103,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "76a9d7f0bce0e2a1dd271b106801325beb866dc3",
          "message": "docs: clarify CORE-MATH-style project scope",
          "timestamp": "2026-08-25T02:37:29+08:00",
          "tree_id": "e13764e5947fcc54f487606394772138763713c2",
          "url": "https://github.com/jdh8/metallic-rs/commit/76a9d7f0bce0e2a1dd271b106801325beb866dc3"
        },
        "date": 1787598910646,
        "tool": "cargo",
        "benches": [
