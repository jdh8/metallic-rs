...
`expm1q` shares that engine and its tables but not its frame: `2^f − 1` is exact
yet keeps only `128 − |log2 x|` significant bits, so near zero it multiplies the
whole 256-bit reduced fraction by the Taylor slope `(2^y − 1)/y` instead.  Its
accurate leg there forms `e^x − 1 = x·G(x)` as an exact product with the *input*
